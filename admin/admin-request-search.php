...
if (isset($_GET['setName'])) {
    $name = $_GET['name'];
    $userRequest = $db->query("SELECT * FROM users WHERE (unblock_request = 'yes' OR writing_request = 'yes') AND name LIKE '%$name%' ORDER BY id DESC");
} elseif (isset($_GET['setType'])) {
    $type = $_GET['type'];
    if ($type != 'all') {
        $userRequest = $db->query("SELECT * FROM users WHERE (unblock_request = 'yes' OR writing_request = 'yes') AND type='$type' ORDER BY id DESC");
    } else {
    ...
    $date = $_GET['date'];
    $userRequest = $db->query("SELECT * FROM users WHERE (unblock_request = 'yes' OR writing_request = 'yes') AND date='$date' ORDER BY id DESC");
} else {
...
<div class="container text-center mt-5">
    <a href="./admin-request.php" class="btn btn-outline-secondary">بازگشت</a>
</div>
