...
if (isset($_GET['setTitle'])) {
    $title = $_GET['title'];
    $news = $db->query("SELECT * FROM news WHERE title LIKE '%$title%' ORDER BY id DESC");
} elseif (isset($_GET['setWriter'])) {
    $writer = $_GET['writer'];
    $news = $db->query("SELECT * FROM news JOIN users ON news.writer_id = users.id WHERE users.name LIKE '%$writer%' ORDER BY news.id DESC");
} elseif (isset($_GET['setDate'])) {
    $date = $_GET['date'];
    $news = $db->query("SELECT * FROM news WHERE date='$date' ORDER BY id DESC");
} else {
...
<div class="container text-center mt-5">
    <a href="./admin-news.php" class="btn btn-outline-secondary">بازگشت</a>
</div>
