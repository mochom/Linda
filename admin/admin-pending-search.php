...
if (isset($_GET['setTitle'])) {
    $title = $_GET['title'];
    $pendingNews = $db->query("SELECT * FROM news WHERE status='pending' AND title LIKE '%$title%' ORDER BY id DESC");
} elseif (isset($_GET['setWriter'])) {
    $writer = $_GET['writer'];
    $pendingNews = $db->query("SELECT * FROM news JOIN users ON news.writer_id = users.id WHERE news.status='pending' AND users.name LIKE '%$writer%' ORDER BY news.id DESC");
} elseif (isset($_GET['setDate'])) {
    $date = $_GET['date'];
    $pendingNews = $db->query("SELECT * FROM news WHERE status='pending' AND date=$date ORDER BY id DESC");
} else {
...
<div class="container text-center mt-5">
    <a href="./admin-pending.php" class="btn btn-outline-secondary">بازگشت</a>
</div>
